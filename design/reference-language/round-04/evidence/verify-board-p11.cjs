...
(async () => {
  ...
  await app.screenshot({ animations: 'disabled', path: path.join(__dirname, 'p11-home-final-390.png') });
  const boxes = await app.locator('.header,.search').evaluateAll(es => es.map(e => e.getBoundingClientRect().toJSON()));
  await app.screenshot({ animations: 'disabled', path: path.join(__dirname, 'p11-Q1-final.png'), clip: { x: 0, y: boxes[0].y, width: 390, height: boxes[1].bottom - boxes[0].y } });
  ...
  for (const region of regions) {
    ...
    await app.screenshot({ animations: 'disabled', path: path.join(__dirname, `p11-${id}-final.png`), clip: { x: 0, y: rect.y, width: 390, height: rect.height } });
  }
  await app.getByRole('button', { name: '온실 커피 9월 12일 13:00 예약 선택', exact: true }).click();
  await app.locator('dialog').screenshot({ animations: 'disabled', path: path.join(__dirname, 'p11-Q5-final.png') });
  await app.close();
  const results = [];
  for (const width of [1100, 390, 320]) {
    ...
    await page.screenshot({ animations: 'disabled', path: path.join(__dirname, `p11-board-${width}.png`), fullPage: true });
    if (width === 1100) { await page.locator('.preview').screenshot({ animations: 'disabled', path: path.join(__dirname, 'p11-preview-final.png') }); for (const q of ['Q1','Q2','Q3','Q4','Q5']) await page.locator(`#${q}`).screenshot({ animations: 'disabled', path: path.join(__dirname, `p11-${q}-comparison.png`) }); }
    results.push({ detail, state, filtered, opened, closed, disclosures, focus });
    await page.close();
  }
  fs.writeFileSync(path.join(__dirname, 'p11-board-checks.json'), JSON.stringify(results, null, 2) + '\n');
  await browser.close();
