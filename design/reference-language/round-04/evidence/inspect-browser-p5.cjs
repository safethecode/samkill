...
async function inspect(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      text: visible.filter(element=>[...element.childNodes].some(node=> node.nodeType===Node.TEXT_NODE && node.textContent?.trim())).map(element=>({tag:element.tagName,class:element.className,text:element.textContent?.trim(),compact:!!element.closest(".admission-tag,.cafe-note"),size:getComputedStyle(element).fontSize,color:getComputedStyle(element).color,...box(element)})),
