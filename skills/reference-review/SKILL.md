...
[UI 구성 기준](../reference-to-ui/references/ui-composition.md)을 읽고 RUI-08–15를 실제 화면/코드/행동과 대조한다. 로고에 실제 요청 근거가 있는지, 14px 하한, CTA와 필터의 구분, 문구의 필요 위치, 아이콘 출처·의미, 반복 구분선, 소수 항목 스크롤, 목록/채팅 전환을 검사한다. 구현자가 쓴 ‘브랜드’, ‘가독성’, ‘주 콘텐츠라 스크롤 허용’이라는 설명만으로 통과시키지 않는다.
